...
class DoLa():
    ...
    def load_model(self, model_name_path):
        ...
        model = AutoModelForCausalLM.from_pretrained(model_name_path,  **kwargs)
    ...
    def set_stop_words(self, stop_words):
        ...
        for stop_word in self.stop_words:            
            stop_word_ids = self.tokenizer.encode(stop_word, add_special_tokens=False)
            list_stop_word_ids.append(stop_word_ids)
            print("Added stop word:", stop_word, "with the ids", stop_word_ids, flush=True)
        self.stopping_criteria.append(LLamaQaStoppingCriteria(list_stop_word_ids))
        print("Stopping criteria:", self.stopping_criteria)
    ...
    def generate(self, input_text, max_new_tokens=256, top_p=0.95, top_k=0, temperature=0.8, mature_layer=None, premature_layer=None, candidate_premature_layers=[], mode='baseline', verbose=True, remove_stop_words=False, relative_top=0.1, **kwargs):
        with torch.no_grad():
            if isinstance(input_text, str):
                ...
                input_ids = input_text.to(self.device)
