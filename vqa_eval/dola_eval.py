...
def find_early_layers(early_exit_layers, repetition_penalty):
    ...
    return mode, mature_layer, premature_layer, candidate_premature_layers
...
def llm_generate_answers(args, rank):
    ...
    system_prompt = "This is a question and answer task. Please analyze the provided Tags, Attributes, and Captions to deduce the short answer to the given question. Ensure your response is succinct, limited to one word or phrase, and directly relates to the information provided. "
    ...
    if args.llama2_7b:
        with torch.no_grad():
            try:
                ...
                return []                           
            predictions = [] 
            for batch in tqdm(args.small_dataset, desc='Processing batches'):
                ids = batch['question_id']
                images, questions, question_types = batch['image'], batch['question'], batch['question_type'] 
                # effective_num_shots = compute_effective_num_shots(args.num_shots)
                ...
                stop_words = ['</s>']
                args.llm.set_stop_words(stop_words) 
                mode, mature_layer, premature_layer, candidate_premature_layers = find_early_layers(args.early_exit_layers, args.repetition_penalty)
                print(mode)
                for icl_example, sample_id in zip(icl, ids): 
                    generate_kwargs = dict(max_new_tokens=args.max_new_tokens, do_sample=args.do_sample, top_p=args.top_p, top_k=args.top_k, temperature=args.temperature, repetition_penalty=args.repetition_penalty, mode=mode, mature_layer=mature_layer, base_layer=premature_layer, premature_layer=premature_layer, candidate_premature_layers=candidate_premature_layers, relative_top=args.relative_top)
                    result, premature_layer_dist = args.llm.generate(icl_example, **generate_kwargs)            
                    pred = result.replace("\n", "").replace("\\", "").strip() 
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(args.small_dataset, desc='Processing batches'):
                ...
                print(icl)
                stop_words = ['<|end_header_id|>']  
                args.llm.set_stop_words(stop_words)  
                mode, mature_layer, premature_layer, candidate_premature_layers = find_early_layers(args.early_exit_layers, args.repetition_penalty)
                print(mode)
                for message, sample_id in zip(icl, ids):
                    icl_example, terminators = args.llm.transform_message(message)
                    generate_kwargs = dict(max_new_tokens=args.max_new_tokens, do_sample=args.do_sample, top_p=args.top_p, top_k=args.top_k, temperature=args.temperature, repetition_penalty=args.repetition_penalty, mode=mode, mature_layer=mature_layer, base_layer=premature_layer, premature_layer=premature_layer, candidate_premature_layers=candidate_premature_layers, relative_top=args.relative_top)
                    result, premature_layer_dist = args.llm.generate(icl_example, **generate_kwargs)
                    pred = result.replace("\n", "").replace("\\", "").strip() 
                    match = re.search(r':(.+)', pred)
...
def main(rank, world_size):
    ...
    parser.add_argument("--query_set_size", default=100, type=int)
    ...
    parser.add_argument("--qwen2_7b_path", type=str, default="../models/Qwen2-7B-Instruct")
    parser.add_argument("--qwen2_7b", action="store_true")
    parser.add_argument("--no_cuda", default=False, action='store_true')
    parser.add_argument("--num_gpus", default=1, type=int) 
    parser.add_argument("--max_gpu_memory", default=27, type=int)
    parser.add_argument("--premature_layer", type=int, default=4, help="Layer is be quited.")
    ...
    parser.add_argument("--instance_id", type=int, default=4, help="Unique identifier for this script instance.")
    parser.add_argument("--early_exit_layers", type=str, default=-1, help="Determin early exit layer")
    ...
    ################################  download model  ##############################################
    if args.llama2_7b:
        args.llm = DoLa(args.llama_model_name_or_path, args.device, args.num_gpus, args.max_gpu_memory)  

    elif args.llama2_13b:
        args.llm = DoLa(args.llama_model_path, args.device, args.num_gpus, args.max_gpu_memory)
      
    elif args.llama3_8b:
        args.llm = DoLa(args.llama3_8b_instruct_path, args.device, args.num_gpus, args.max_gpu_memory) 
    elif args.qwen2_7b:
        args.llm = DoLa(args.qwen2_7b_path, args.device, args.num_gpus, args.max_gpu_memory) 
    else:
    ...
    ################################################ save results ######################################################################
    with open(f"../result/okqwendola_result_{args.instance_id}.json", "w") as f:
        json.dump(predictions, f, ensure_ascii=False, indent=4)

########################################################################################
    # with open(f"llama2_result_{args.instance_id}_{rank}.json", "w") as f:
    #     json.dump(predictions, f, ensure_ascii=False, indent=4)
    ...
    #         with open(f"llama2_result_{args.instance_id}_{i}.json", "r") as f:
    #             result = json.load(f)
    #             all_result.extend(result)
    #     with open(f"llama2_result_{args.instance_id}.json", "w") as f:
    #         json.dump(all_result, f, ensure_ascii=False, indent=4)
####################################################################################################################
        
    # with open (f"llama_result_{rank}.json", "w") as f:
    #     json.dump(predictions, f, ensure_ascii=False, indent=4)   
    # torch.distributed.barrier()
    ...
    #     all_result = []
    #     for i in range(world_size):
    #         with open(f"llama_result_{i}.json", "r") as f:
    #             result = json.load(f)
    #             all_result.extend(result)
    #     with open("llama_result.json", "w") as f:
    #         json.dump(all_result, f, ensure_ascii=False, indent=4)
