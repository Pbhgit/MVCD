...
class Lens1(nn.Module):
    def __init__(
        self,
        clip_name: str = "../models/laion_CLIP/open_clip_pytorch_model.bin",
        blip_name: str = "../models/blip-image-captioning-large",
        attributes_weights: str = "../weights/attributes_weights/zw_attributes_laion_ViT_H_14_2B_descriptors_text_davinci_003_full.pt",
        tags_weights: str = "../weights/tags_weights/zw_tags_laion_ViT_H_14_2B_vocab_lens.pt",
        vocab_attributes: str = "../vocab/vocab_attributes/",
        vocab_tags: str = "../vocab/vocab_tags/",
    ...
    def load_clip_model(self, model_name: str, device: torch.device):
        if "openai" in model_name:
            ...
            model = open_clip.create_model_and_transforms('ViT-H-14',  pretrained=model_name)[0].to(device)
        return model
    ...
    def create_prompt_from_samples(
        ...
        for idx in range(num_samples):
            prompt, del_tag_prompt, del_attr_prompt, del_caption_prompt = create_prompt_sample(samples, idx, mode=mode)
            prompts.append(prompt)
            del_tag_prompts.append(del_tag_prompt)
            del_attr_prompts.append(del_attr_prompt)
            del_caption_prompts.append(del_caption_prompt)
        samples["prompts"] = prompts
        samples["del_tag_prompts"] = del_tag_prompts
        samples["del_attr_prompts"] = del_attr_prompts
        samples["del_caption_prompts"] = del_caption_prompts
        return samples
...
class LensProcessor:
    ...
    def load_clip_transform(self, model_name: str):
        if "openai" in model_name:
            ...
            return open_clip.create_model_and_transforms('ViT-H-14', model_name)[2]
